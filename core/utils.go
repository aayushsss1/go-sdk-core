...
var rePassword1 = regexp.MustCompile(`(?i)(password|token|apikey|api_key|api-key|secret|key|access|thumbprint|auth|project_id|client_id|client_email|tenantId|subscriptionId|aadClientId|aadClientSecret|client_x509_cert_url)=[^&]*(&|$)`)
var rePassword2 = regexp.MustCompile(`(?i)"([^"]*(password|token|apikey|api_key|api-key|secret|key|access|thumbprint|auth|project_id|client_id|client_email|tenantId|subscriptionId|aadClientId|aadClientSecret|client_x509_cert_url)[^"_]*)":\s*"[^\,]*"`)
